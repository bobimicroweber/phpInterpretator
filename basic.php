 ...
 *
 * @author Bozhidar Slaveykov <http://cloudvision.bg>
 * @version 1.0.0
 * @copyright Copyright (c) 2010 Bozhidar Slaveykov
 * @license MIT License
 * @package basic
 **/
...
define('TOKEN_LEFT_PARENTHESIES',7);
define('TOKEN_RIGHT_PARENTHESIES',8);
define('TOKEN_EOF',9);
...
class Parser {
	...
	public function parse() {
		...
		while (TRUE) {
			...
			else if (isset($this->current()->token) && ($this->current()->token == "echo")) {
			...
			else if (isset($this->current()->token) && $this->current()->token == "input") {
			...
			else if (isset($this->current()->token) && $this->current()->token == "goto") {
			...
			else if (isset($this->current()->token) && $this->current()->token == "exit") {
	...
	public function next($offset = 0) {
        if (!isset($this->tokens[$this->position + 1 + $offset])) {
            return false;
        }
	...
	public function match($token_one, $token_two = FALSE) {
		if (!$token_two) {
			// Compare and return
			if (isset($this->current()->type) && $this->current()->type == $token_one) {
		...
		// We have two tokens
			...
			if (isset($this->current()->type) && $this->current()->type == $token_one) {
				...
				if (isset($this->next()->type) && $this->next()->type == $token_two) {
					// Increment the position
					$this->position++;
